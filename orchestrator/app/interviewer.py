...
class MedicalInterviewer:
    ...
    async def curate_evidence(self, raw_evidence: str, chief_complaint: str) -> str:
        ...
        system = (
            "你是主治医师，正在转诊前整理病历。你的任务是【清理格式】，不是筛选医学内容。规则：\n"
            "1. 逐份报告，完整保留其中每一项客观数值、检查发现、明确诊断与既往史条目"
            "（如血糖/血脂/血压等各项化验值、囊肿/结节/斑块、器官体积或大小、阴性所见等），一项都不得省略或合并丢弃。\n"
            "2. 不要判断某项是否与本次主诉相关——客观信息一律保留，相关性留给后续评估医师判断。\n"
            "3. 只删除：病人ID、检查号、设备型号/参数、机构抬头与落款、页码、完全重复的行。\n"
            "用简洁中文按报告分组条列，每组开头标明来源类型与日期。严禁臆测或新增报告中没有的信息。"
            "若【既往在档报告】确为空，才回复『无相关既往佐证』。"
        )
        user = (
