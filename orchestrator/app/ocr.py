...
def _pdf_text_layer(data: bytes) -> str:
    """数字版 PDF 的文本层：pdftotext 秒出无损，避免 glm-ocr 栅格化的失真/丢字。"""
    try:
        ...
        return out if len(re.sub(r"\s", "", out)) >= 20 else ""
    except Exception:
        return ""
...
async def extract_text(data: bytes, content_type: str) -> str:
    ...
    if "pdf" in ct:
        text_layer = _pdf_text_layer(data)        # 文本层优先（数字版报告秒出无损）
        if text_layer:
            return text_layer.strip()
        pages = _pdf_to_png_pages(data)           # 扫描件才栅格化逐页过 glm-ocr
        parts = []
