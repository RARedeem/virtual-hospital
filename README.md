...
| 类别 | 约束 | 实现位置 |
|---|---|---|
| 模型（约束 A） | 仅非中国大陆机构开发 | gemma4(谷歌) / meditron(瑞士) / nomic(美) |
| 指南数据源（约束 B） | 仅国际权威机构 | `knowledge_base` schema + CHECK 约束 |
...
`OLLAMA_MAX_LOADED_MODELS=2` 允许 27B 翻译模型与 70B 推理模型尽量共驻。显存不足时 Ollama 自动换载，单次评估增加约 20-60 秒加载延迟，家族级使用频率下可接受。
...
> **v2 双盲交叉验证架构（流程A1/A2背靠背、并排呈现）整体仍未实现**：
> 其中**流程 A1 问诊交互（前后端）已于 2026-06-23 落地**（见上「问诊交互（流程 A1 前端）」节），
> 但 A2 国内指南推理、呈现层并排、A/B 双盲整合**尚未实现**。详见 ARCHITECTURE.md。
> 请勿据本节误判 v2 整体已就绪。
